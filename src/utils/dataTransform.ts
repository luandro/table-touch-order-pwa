...
import type { Table as SupabaseTable, Order as SupabaseOrder, MenuItem as SupabaseMenuItem } from '@/types/supabase';
...
export const transformSupabaseTable = (supabaseTable: SupabaseTable): Table => {
  return {
    ...
    lastActivity: supabaseTable.created_at ? new Date(supabaseTable.created_at) : undefined,
  };
};
...
export const transformSupabaseMenuItem = (supabaseItem: SupabaseMenuItem, categoryName: string): MenuItem => {
  return {
    id: supabaseItem.id,
    ...
    image: supabaseItem.image_url || `https://picsum.photos/400/225?random=${Math.floor(Math.random() * 100)}`,
...
const transformOrderItems = (items: any[]): BillItem[] => {
  return items.map((item: any) => ({
    ...
    menuItem: {
      ...
      image: item.image || `https://picsum.photos/400/225?random=${Math.floor(Math.random() * 100)}`,
      rating: 4.5,
      available: true,
    },
...
const mapTableStatus = (status: string): 'free' | 'occupied' | 'pending' | 'reserved' => {
  switch (status) {
...
const mapOrderStatus = (status: string): 'pending' | 'confirmed' | 'preparing' | 'ready' | 'served' => {
  switch (status) {
