...
import {
  historyService,
  type TableHistoryRecord,
  type TableHistoryInsert,
...
export const useLogAction = () => {
  ...
  return useMutation({
    mutationFn: (historyData: TableHistoryInsert) =>
      historyService.logAction(historyData),
    onSuccess: (data) => {
      // Invalidate related queries
      queryClient.invalidateQueries({
        queryKey: ["table-history", data.table_id],
      });
...
export const useLogTableAction = () => {
  return {
    ...
    logCustomerNameUpdated: (
      tableId: string,
      oldName: string,
      newName: string,
      adminUserId?: string,
    ) =>
      historyService.logCustomerNameUpdated(
        tableId,
        oldName,
        newName,
        adminUserId,
...
export const useRealTimeTableHistory = (tableId: string) => {
  ...
  useEffect(() => {
    ...
    const subscription = historyService.subscribeToTableHistory(
      tableId,
      (payload) => {
...
function formatDetailsForDisplay(
  action: string,
